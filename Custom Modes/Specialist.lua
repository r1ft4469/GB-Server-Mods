...
function specialist:CheckEndRoundTimer()
	...
	if SpecialistLives == "0" then
		...
		elseif self.DefendingTeamId == self.RedTeamId then
	...
	elseif #BluePlayers == 0 and #RedPlayers > 0 then
		if self.DefendingTeamId == self.RedTeamId then
			...
			gamemode.AddGameStat("CompleteObjectives=DefendObjective")
			gamemode.SetRoundStage("PostRoundWait")
		elseif self.DefendingTeamId == self.BlueTeamId then
			gamemode.AddGameStat("Result=Team2")
			gamemode.AddGameStat("Summary=BlueEliminated")
			gamemode.AddGameStat("CompleteObjectives=CaptureObjective")
			gamemode.SetRoundStage("PostRoundWait")
...
function specialist:SetupRound()
	if self.AutoSwap then
		...
		if self.AutoSwapCount > 1 then
			...
			if self.AttackingTeamId == self.BlueTeamId then
				BlueMessage = "You are now attacking."
				RedMessage = "You are now defending."
			else
				BlueMessage = "You are now defending."
				RedMessage = "You are now attacking."
			end
			...
			for i = 1, #RedPlayers do
				player.ShowGameMessage(RedPlayers[i], RedMessage, 10.0)
			end
		end
	end
	...
	gamemode.AddGameObjective(self.DefendingTeamId, "Defend Laptop 1.", 1)
	gamemode.AddGameObjective(self.DefendingTeamId, "Defend Laptop 2.", 1)
	gamemode.AddGameObjective(self.DefendingTeamId, "Kill The Specialist.", 2)
	gamemode.AddGameObjective(self.AttackingTeamId, "Escort The Specialist To Laptop 1 or Laptop 2.", 1)
	gamemode.AddGameObjective(self.AttackingTeamId, "Cover Specialist During Extraction of Intel", 1)
	gamemode.AddGameObjective(self.AttackingTeamId, "Kill All Resistance.", 2)
	...
	local RandomLaptopIndex0 = umath.random(#self.GroupedLaptops);
	local RandomLaptopIndex1 = umath.random(#self.GroupedLaptops);
	if RandomLaptopIndex0 == RandomLaptopIndex1 then
		if RandomLaptopIndex0 > 1 then
			RandomLaptopIndex1 = RandomLaptopIndex1 - 1
		else
			RandomLaptopIndex1 = RandomLaptopIndex1 + 1
		end
	end
	self.RandomLaptop0 = PossibleLaptops[RandomLaptopIndex0]
	self.RandomLaptop1 = PossibleLaptops[RandomLaptopIndex1]
	for Group, Laptops in pairs(self.GroupedLaptops) do
		for j, Laptop in ipairs(Laptops) do
			local bActive = (Laptop == self.RandomLaptop1 or Laptop == self.RandomLaptop0)
...
function specialist:TargetCaptured()
	...
	if LocationDist0 < 10000  or LocationDist1 < 10000 then 
		...
		for d = 1, #DefendingPlayers do
			player.ShowGameMessage(DefendingPlayers[d], "Specialist secured the intel.", 2.0)
		end
		...
		for a = 1, #AttackingPlayers do
			player.ShowGameMessage(DefendingPlayers[d], "Non-specialist tried to secured the intel.", 2.0)
		end
		for d = 1, #DefendingPlayers do
			player.ShowGameMessage(DefendingPlayers[d], "Non-specialist tried to secured the intel.", 2.0)
		end
...
function specialist:PlayerEnteredPlayArea(PlayerState)
	...
	elseif actor.GetTeamId(PlayerState) == self.DefendingTeamId then
		player.ShowGameMessage(PlayerState, "Defend the intel.", 5.0)
		local LaptopLocation0 = actor.GetLocation(self.RandomLaptop0)
		local LaptopLocation1 = actor.GetLocation(self.RandomLaptop1)
		player.ShowWorldPrompt(PlayerState, LaptopLocation0, "DefendTarget", self.DefenderSetupTime - 2)
		player.ShowWorldPrompt(PlayerState, LaptopLocation1, "DefendTarget", self.DefenderSetupTime - 2)
	end
end
